#!/usr/bin/env python3
"""
Session Log Reminder Hook for Codex

Optional utility hook that tracks how many responses have passed since the
session log was last updated. After a threshold, it blocks Codex from
stopping and reminds it to update the session log.

Adapted from: https://gist.github.com/michaelewens/9a1bc5a97f3f9bbb79453e5b682df462

Usage (external hook runner example):
    python3 "$CODEX_PROJECT_DIR/.codex/hooks/log-reminder.py"
"""
...
THRESHOLD = 15
STATE_DIR = Path("/tmp/codex-log-reminder")
...
def get_project_dir():
    ...
    # If stop_hook_active, Codex is already continuing from a previous
...
def main():
    ...
    # Case 1: No session log exists at all — remind once, then let Codex work
    if latest_log is None:
        ...
        # Already reminded — let Codex proceed (it will create the log)
        sys.exit(0)
...
if __name__ == "__main__":
    try:
        ...
        # Fail open — never block Codex due to a hook bug
        sys.exit(0)
