...
- **Context loss between sessions.** You pick up where you left off, but Codex doesn't remember *why* you chose that notation, *what* the instructor approved, or *which* bugs were fixed last time.
...
This workflow solves all of these problems. You describe what you want --- "translate Lecture 5 to Quarto" or "create a new lecture on synthetic control" --- and Codex handles the rest: plans the approach, implements it, runs specialized reviewers, fixes issues, verifies quality, and presents results. Like a contractor who manages the entire job.

## What Makes Codex Different

Codex runs on your computer with full access to your file system, terminal, and git. It works as a **CLI tool**, a **VS Code extension**, or through **ChatGPT Codex sessions** --- same capabilities, same configuration, different interface. Here is what that enables:
...
| Orchestrator mode | Codex autonomously plans, implements, reviews, fixes, and verifies |
...
You need two things: fork the repo, and paste a prompt. Codex handles everything else.
...
## Step 2: Start Codex and Paste This Prompt {#sec-first-session}

Open your terminal in the project directory, run `codex`, and paste the following. Fill in the **bolded placeholders** with your project details:

::: {.callout-note collapse="true"}
## Using VS Code or ChatGPT instead of the terminal?

Everything in this guide works the same in any Codex interface. In **VS Code**, open the Codex panel (click the Codex icon in the sidebar or press Cmd+Shift+P → "Codex: Open"). In **ChatGPT**, open a Codex session for your project folder. Then paste the starter prompt below.
...
**What this does:** Codex will read `AGENTS.md` and all the rules, fill in your project name, institution, Beamer environments, CSS classes, and project state table, then propose any rule adjustments for your specific use case. You approve the plan, and Codex handles the rest. From there, you just describe what you want to build.

## Optional: Manual Setup

If you prefer to configure things yourself instead of letting Codex handle it:

::: {.callout-note collapse="true"}
## Manual Configuration Steps (click to expand)

**Customize AGENTS.md** --- Open `AGENTS.md` and replace all `[BRACKETED PLACEHOLDERS]`:
...
**Create your knowledge base** --- Open `.codex/rules/knowledge-base-template.md` and fill in:
...
**Configure Codex settings** --- Review `.codex/config.toml`. The template includes model, approval, sandbox, and web-search settings. Add any project-specific adjustments you need.
...
Consider proofreading a 140-slide lecture deck. You could ask Codex:

> "Review these slides for grammar, layout, math correctness, code quality, and pedagogical flow."

Codex will skim everything and catch some issues. But it will miss:
...
**Why it works:** The critic can't fix files (read-only), so it has no incentive to downplay issues. The fixer can't approve itself (the critic re-audits). This prevents the common failure of Codex saying "looks good" about its own work.
...
The **orchestrator protocol** (`.codex/rules/orchestrator-protocol.md`) is an auto-loaded rule that activates after any plan is approved. It implements the plan, runs the verifier, selects review agents based on file types, applies fixes, re-verifies, and scores against quality gates. It loops until the score meets threshold or max rounds are exhausted.
...
The verification protocol (`.codex/rules/verification-protocol.md`) requires that Codex compile, render, or otherwise verify every output before reporting a task as complete. The orchestrator enforces this as an explicit step in its loop (Step 2: VERIFY). This means Codex **cannot** say "done" without actually checking the output.
...
To customize, open `.codex/agents/domain-reviewer.md` and fill in:
...
Understanding the configuration layers helps you customize the workflow and debug when things go wrong. Codex's power comes from five configuration layers that work together --- think of them as the operating system for your academic project.

## AGENTS.md --- Your Project's Constitution

`AGENTS.md` is the single most important file. Codex reads it at the start of every session. But here is the critical insight: **Codex reliably follows about 100--150 custom instructions.** Your system prompt already uses ~50, leaving ~100--150 for your project. AGENTS.md and always-on rules share this budget.
...
Move everything else into `.codex/rules/` files (with path-scoping so they only load when relevant).
...
AGENTS.md loads every session. If it exceeds ~150 lines, Codex starts ignoring rules silently. Put detailed standards in path-scoped rules (`.codex/rules/`) instead --- they only load when Codex works on matching files, so they don't compete for attention.
:::

## Rules --- Domain Knowledge That Auto-Loads

Rules are markdown files in `.codex/rules/` that Codex loads automatically. They encode your project's standards. The key design principle is **path-scoping**: rules with a `paths:` YAML frontmatter only load when Codex works on matching files.
...
This design keeps always-on context under ~100 lines while providing rich, domain-specific guidance exactly when Codex needs it.

**Why rules matter:** Without them, Codex will use generic defaults. With them, Codex follows *your* standards consistently across sessions.
...
The `paths:` block means this rule only loads when Codex reads or edits an `.R` file. When Codex works on a `.tex` file, this rule doesn't consume any of the instruction budget.

## Skills --- Reusable Slash Commands

Skills are multi-step workflows invoked with `/command`. Each skill lives in `.codex/skills/[name]/SKILL.md`:
...
metadata:
  short-description: Compile LaTeX slides with 3-pass XeLaTeX + bibtex
---
...
A single Codex prompt trying to check grammar, layout, math, and code simultaneously will do a mediocre job at all of them. Specialized agents focus on one dimension and do it thoroughly. The `/slide-excellence` skill runs them all in parallel, then synthesizes results.
:::
...
## Settings --- Config and Sandbox

`.codex/config.toml` controls the project-level Codex runtime defaults (model, approvals, sandbox mode, and web search). Keep this file concise and intentional:
...
You can also keep optional utility scripts in `.codex/hooks/` for external runners (for example, a local wrapper script or editor task). They are not required by Codex itself.

## Memory --- Cross-Session Persistence

This template uses repository-level memory at `MEMORY.md`. Keep durable project facts and correction tags there so they remain available across sessions.
...
Git commits record what changed, but not *why*. Session logs fill this gap. Codex writes to `quality_reports/session_logs/` at three points: right after plan approval, incrementally during implementation (as decisions happen), and at session end. This means the log captures reasoning *as it happens*, before auto-compression can discard it.

Because relying on instructions alone is fragile (Codex forgets during long sessions), this template includes an optional reminder utility (`.codex/hooks/log-reminder.py`). If you wire it into your own external hook runner, it can track how many responses have passed since the session log was last updated and remind you when logging is overdue.
...
With AGENTS.md, MEMORY.md, plans, and session logs, the system has four distinct memory layers. Here is what each one does and when it matters:

| Layer | File | Survives Compression? | Updated When | Purpose |
|-------|------|----------------------|--------------|---------|
| Project context | `AGENTS.md` | Yes (on disk) | Rarely | Project rules, folder structure, commands |
...
| Conversation | Codex's context window | **No** (compressed) | Every response | Current working memory |
...
The session-log reminder above is one example of a broader pattern: use lightweight scripts for mechanical checks, and use `.codex/rules/` for nuanced workflow behavior.

The template includes four optional utility scripts:

| Script | Purpose |
|------|-------------|
| `.codex/hooks/log-reminder.py` | Reminds when session logs are stale |
| `.codex/hooks/notify.sh` | Sends macOS desktop notifications |
| `.codex/hooks/protect-files.sh` | Blocks accidental edits to protected files |
| `.codex/hooks/pre-compact.sh` | Appends a context snapshot before compaction |

Verification and Beamer-Quarto sync are enforced via auto-loaded rules (`.codex/rules/verification-protocol.md` and `.codex/rules/beamer-quarto-sync.md`), which are the right tool for behavioral guidance that Codex can evaluate in-context.

::: {.callout-tip}
## Hook Design Principle
Use **command-based hooks** for fast, mechanical checks (file exists? counter threshold?). Use **rules** for nuanced judgment (did Codex verify correctly?). Avoid prompt-based hooks that trigger an LLM call on every response --- the latency adds up fast.
:::
...
- Codex starts editing immediately, discovers a dependency on slide 3 that changes the approach, and has to undo work
- Context compression discards the reasoning behind a design choice, and Codex makes a contradictory decision later
- The user and Codex have different mental models of what "done" looks like
...
An important companion to plan-first development is context preservation. Codex has two ways to free context space:

- **`/clear`** --- destroys everything. All context, corrections, and reasoning are gone. Starting over from zero.
- **Auto-compression** --- Codex automatically compresses the conversation when it gets long. It keeps the most important context and discards redundant detail. This is graceful degradation.
...
If you notice Codex has lost context after auto-compression, point it to the saved plan file:
...
Codex writes all three log entries automatically --- no need to ask.
...
These tags are searchable and persist in MEMORY.md across sessions. When Codex encounters a similar situation, it checks memory first.
...
Codex can spawn **multiple agents simultaneously** using the Task tool. This is not limited to review --- you can use it for any research or analysis task where independent subtasks can run at the same time.
...
Either way, Codex spawns up to 3 Task agents, each processing one paper simultaneously, then synthesizes the results.
...
The **kill switch** is explicit: at any point, you can stop, archive with a one-paragraph explanation, and move on. No guilt, no sunk cost. See `.codex/rules/exploration-folder-protocol.md` and `.codex/rules/exploration-fast-track.md` for the full protocols.

#### Simplified Orchestrator for Research

The full orchestrator (Pattern 2) is designed for course materials with multi-agent review loops. For research projects, the **simple variant** strips this down to: implement → verify → score → done. No multi-round reviews, no parallel agent spawning. This lives in its own path-scoped rule (`.codex/rules/orchestrator-research.md`) that loads only when working on R scripts or explorations.

#### Merge-Only Quality Reporting

In research projects, commits are frequent and incremental. Generating a quality report for each commit creates noise. Instead, quality reports are generated **only at merge time** --- a permanent snapshot of what was merged and why. Session logs capture the ongoing reasoning. See `.codex/rules/session-logging.md`.
...
Copy `.codex/agents/domain-reviewer.md` and customize the 5 lenses for your field. The template provides the structure; you fill in domain-specific checks.
...
Use `.codex/skills/proofread/SKILL.md` as a starting template, then copy its structure into your new skill folder and adapt the workflow steps.

## Tips from 6+ Sessions of Iteration

1. **Keep AGENTS.md under 150 lines.** Codex follows ~150 instructions reliably. A 400-line AGENTS.md means rules get silently ignored. Use path-scoped rules for detailed standards.
...
8. **Progressive disclosure.** Start with AGENTS.md + 2--3 rules. Add more as your workflow matures. Newcomers should not face 17 rules on day one.
...
| Agent | File | Purpose |
|-------|------|---------|
| Proofreader | `.codex/agents/proofreader.md` | Grammar, typos, consistency |
| Slide Auditor | `.codex/agents/slide-auditor.md` | Visual layout, overflow, spacing |
| Pedagogy Reviewer | `.codex/agents/pedagogy-reviewer.md` | Narrative arc, notation clarity |
| R Reviewer | `.codex/agents/r-reviewer.md` | R code quality, reproducibility |
| TikZ Reviewer | `.codex/agents/tikz-reviewer.md` | Diagram visual quality |
| Beamer Translator | `.codex/agents/beamer-translator.md` | LaTeX to Quarto translation |
| Quarto Critic | `.codex/agents/quarto-critic.md` | Adversarial Quarto QA |
| Quarto Fixer | `.codex/agents/quarto-fixer.md` | Applies critic's fixes |
| Verifier | `.codex/agents/verifier.md` | Task completion verification |
| Domain Reviewer | `.codex/agents/domain-reviewer.md` | Your domain-specific review |

## All Skills

| Skill | Directory | Purpose |
|-------|-----------|---------|
| `/compile-latex` | `.codex/skills/compile-latex/` | XeLaTeX 3-pass compilation |
| `/deploy` | `.codex/skills/deploy/` | Quarto render + GitHub Pages sync |
| `/extract-tikz` | `.codex/skills/extract-tikz/` | TikZ to SVG conversion |
| `/proofread` | `.codex/skills/proofread/` | Run proofreading agent |
| `/visual-audit` | `.codex/skills/visual-audit/` | Run layout audit agent |
| `/pedagogy-review` | `.codex/skills/pedagogy-review/` | Run pedagogy review agent |
| `/review-r` | `.codex/skills/review-r/` | Run R code review agent |
| `/qa-quarto` | `.codex/skills/qa-quarto/` | Critic-fixer adversarial loop |
| `/slide-excellence` | `.codex/skills/slide-excellence/` | Combined multi-agent review |
| `/translate-to-quarto` | `.codex/skills/translate-to-quarto/` | Beamer to Quarto translation |
| `/validate-bib` | `.codex/skills/validate-bib/` | Bibliography validation |
| `/devils-advocate` | `.codex/skills/devils-advocate/` | Design challenge questions |
| `/create-lecture` | `.codex/skills/create-lecture/` | Full lecture creation |
| `/commit` | `.codex/skills/commit/` | Stage, commit, PR, and merge |
| `/lit-review` | `.codex/skills/lit-review/` | Literature search and synthesis |
| `/research-ideation` | `.codex/skills/research-ideation/` | Research questions and strategies |
| `/interview-me` | `.codex/skills/interview-me/` | Interactive research interview |
| `/review-paper` | `.codex/skills/review-paper/` | Manuscript review |
| `/data-analysis` | `.codex/skills/data-analysis/` | End-to-end R analysis |
...
|------|------|--------------|
| Session log reminder | Stop (command) | `.codex/hooks/log-reminder.py` |
| Desktop notification | Notification (command) | `.codex/hooks/notify.sh` |
| File protection | PreToolUse (command) | `.codex/hooks/protect-files.sh` |
| Context snapshot | PreCompact (command) | `.codex/hooks/pre-compact.sh` |
